...
*   **AIによる文字起こしと要約:** Whisper（ローカル）またはGemini APIで文字起こし、Claude CLI / Gemini APIで校正・要約を行います。
*   **Markdown形式での保存:** 要約結果をMarkdownファイルとして保存します。ファイル名は録音日時（`YYYY-MM-DD-HH-MM-SS_タイトル.md`）と内容に基づき自動生成されます。
*   **長時間音声対応:** 20分を超える音声ファイルは自動的に分割処理され、APIの制限に対応しつつ途切れない文字起こし結果を得られます。
*   **キーワードトリガー機能:** 録音中に特定のキーワードを話すと、処理後に自動でアクションを実行します。
    *   `おぶし` → Obsidian vaultのvoiceメモフォルダにノートを作成（ダイアログで確認）
    *   `タスク` → リマインダーに追加（リスト選択ダイアログ）＋「未整理」リストへも自動追加。カレンダーへの翌日7時イベント追加も選択可能
...
2.  **設定ファイルの編集:**
    *   `script/config.sh` を開き、お使いの環境に合わせて以下の項目を設定してください。
        ...
        *   `OBSIDIAN_VAULT_DIR`: Obsidian vaultのvoiceメモフォルダパス（`おぶし`キーワード検出時にノートを作成）。
...
6.  **動作確認 (任意):**
    *   サンプル音声ファイルを `audio/` ディレクトリに直接置き、以下のコマンドで手動実行できます。
        ...
        cd script && python3 transcribe_summarize.py \
          ...
          --transcript_output_dir ../mojitxt \
          --obsidian_vault_dir "/path/to/obsidian/voiceメモ" \
...
- **キーワードトリガー**: 録音中に`おぶし`や`タスク`と話すだけで各アクションを自動実行
    - `おぶし [内容]` → Obsidian vaultにノートを作成（ダイアログで確認）
    - `タスク [内容]` → リマインダーに追加（既存リストから選択）＋「未整理」リストへ自動追加
    - タスクはカレンダーへの翌日7時イベント追加も選択可能
    - Whisperの誤認識（「オプシ」「オブシ」等）にも対応したバリアント検出
    - セパレータなし・前後に雑音があっても検出可能
